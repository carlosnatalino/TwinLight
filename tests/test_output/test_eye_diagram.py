...
class TestSynthesizeEye:
    ...
    def test_high_gsnr_has_clear_eye(self) -> None:
        ...
        result = synthesize_eye(measurements, "DP-QPSK", n_traces=200)
    ...
    def test_pmd_adds_jitter(self) -> None:
        ...
        result_with_pmd = synthesize_eye(
            {"gsnr-db": 30.0, "pmd-ps": 5.0},
            "DP-QPSK",
            n_traces=n_traces,
        )
        cross_idx = int(result_no_pmd.traces.shape[1] * 0.25)
        var_no_pmd = np.var(result_no_pmd.traces[:, cross_idx])
        var_with_pmd = np.var(result_with_pmd.traces[:, cross_idx])
        assert var_with_pmd >= var_no_pmd, "PMD should add timing spread"
