...
Plus a few `twinlight_info{backend=...}` static gauges so dashboards
...
def _make_registry(ctx: Any, cfg: Any) -> CollectorRegistry:
    ...
    info = Gauge(
        "twinlight_info", "Build / backend identity (value is always 1)",
...
def _populate_config_gauges(
    ...
    if not backend.available:
        return
    from twinlight.physics.backend import element_kind_name
    ...
    for attr, name, help_text in _FIBER_ATTRS:
        fiber_metrics[attr] = Gauge(
            f"twinlight_{name}", help_text, ["uid"], registry=registry,
        )
...
async def _populate_opm_gauges(
    ...
    opm_gauges: dict[str, Gauge] = {
        key: Gauge(
            f"twinlight_{name}", help_text, _OPM_LABELS, registry=registry,
        )
        for key, (name, help_text) in _OPM_SPEC.items()
    }
    status_gauge = Gauge(
        "twinlight_service_link_failed",
