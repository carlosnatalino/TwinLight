...
GNPy does not provide spectrum allocation, so this module implements a
first-fit contiguous slot search along a path: find the lowest-indexed
block of contiguous slots (plus guard bands) that is free on every link
of the path. First-fit is the standard baseline heuristic for spectrum
assignment in elastic optical networks; the per-link "available spectrum
slots" abstraction follows optical-networking-gym
(https://github.com/carlosnatalino/optical-networking-gym).
"""

from __future__ import annotations

from twinlight.state.spectrum_state import Edge, SpectrumState
