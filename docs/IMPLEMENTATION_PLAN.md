...
│
├── tapi_client/                          # Companion client library
│   ├── __init__.py
...
```bash
# Normal start
python -m tapi_twin.main --topology topology.json --equipment equipment.json

# Start from a saved snapshot
python -m tapi_twin.main --topology topology.json --equipment equipment.json --restore snapshots/latest.json
```

**Files to create/modify**:

- `src/tapi_twin/api/admin.py` — Admin endpoints (snapshot, restore, latest)
- `src/tapi_twin/state/context.py` — Add `snapshot()` and `restore_from()` methods
- `src/tapi_twin/state/persistence.py` — Serialization helpers (NumPy→base64, event queue→JSON)
- Modify `src/tapi_twin/main.py` — Add `--restore` CLI argument
...
- `src/tapi_twin/models/common.py` — TAPI common types (UUID, AdminState, NameAndValue)
- `src/tapi_twin/models/topology.py` — Topology, Node, Link, NEP, SIP
- `src/tapi_twin/state/context.py` — TapiContext singleton
- `src/tapi_twin/state/topology_state.py` — NetworkX DiGraph wrapper
- `src/tapi_twin/state/spectrum_state.py` — NumPy slot arrays
- `src/tapi_twin/loader/gnpy_topology.py` — Parse GNPy JSON → internal state
- `src/tapi_twin/loader/equipment.py` — Parse GNPy equipment config
- `src/tapi_twin/loader/tapi_builder.py` — Internal state → TAPI model objects
- `src/tapi_twin/api/topology.py` — GET topologies, nodes, links, NEPs
- `src/tapi_twin/api/common.py` — GET context, SIPs
- `src/tapi_twin/api/middleware.py` — RESTCONF content-type
- `src/tapi_twin/app.py` — FastAPI factory
- `src/tapi_twin/config.py` — Settings
...
- `src/tapi_twin/physics/gnpy_adapter.py` — Core GNPy integration
- `src/tapi_twin/physics/steady_state.py` — Multi-lightpath QoT orchestration
- `src/tapi_twin/physics/modulation.py` — Modulation format library
- `src/tapi_twin/physics/ber_conversion.py` — GSNR → BER → Q-factor
...
- `src/tapi_twin/algorithms/routing.py`
- `src/tapi_twin/algorithms/spectrum_assignment.py`
- `src/tapi_twin/algorithms/modulation_selection.py`
- `src/tapi_twin/algorithms/rmsa.py`
- `src/tapi_twin/state/connection_state.py`
- `src/tapi_twin/api/connectivity.py`
- `src/tapi_twin/api/path_computation.py`
- `src/tapi_twin/models/connectivity.py`
- `src/tapi_twin/models/path_computation.py`
...
- `src/tapi_twin/state/context.py` — EdfaStateTracker wiring (add/delete service)
- `src/tapi_twin/api/internal.py` — passes edfa_tracker to all OPM endpoints
- `src/tapi_twin/config.py` — new EDFA config fields (gain_per_channel_db, tau_add_factor, tau_drop_factor)
...
- `src/tapi_twin/output/opm_metrics.py`
- `src/tapi_twin/output/eye_diagram.py`
- `src/tapi_twin/output/constellation.py`
- `src/tapi_twin/output/text_logs.py`
- `src/tapi_twin/api/internal.py` — Internal OPM endpoints (`/internal/opm/{uuid}`) used by gNMI adapter
- `src/tapi_twin/streaming/gnmi_service.py` — Stateless gNMI Subscribe RPC (queries FastAPI via ASGITransport)
- `src/tapi_twin/streaming/measurement_sampler.py` — Periodic polling loop + JSON→protobuf translation
- `src/tapi_twin/streaming/proto/` — Generated protobuf stubs
- `src/tapi_twin/grpc_server.py` — gRPC server setup (receives FastAPI app reference, not TapiContext)
- `src/tapi_twin/api/streaming.py` — WebSocket fallback for OPM
- `src/tapi_twin/api/diagrams.py`
...
- `src/tapi_twin/simulation/clock.py`
- `src/tapi_twin/simulation/event_engine.py`
- `src/tapi_twin/main.py`
- `tapi_client/client.py`, `tapi_client/topology.py`, `tapi_client/connectivity.py`, `tapi_client/streaming.py`
...
python -m tapi_twin.main --topology examples/topology.json --equipment examples/equipment.json
...
| 10 | **Companion Python Client Library** | `tapi_client` package with async httpx client, topology/connectivity/streaming helpers. Enables programmatic access and scripting. | (project plan) | Medium |
