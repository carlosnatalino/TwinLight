  ...
  Transform to Cytoscape elements:
    ...
    • TAPI Link → cy edge {source, target, ...data}
      (resolve node-edge-point refs → parent node UUIDs)
        ...
        └──► localStorage "twinlight-ui:topology" (JSON + timestamp)
```

**On app startup:**
1. Read `localStorage["twinlight-ui:topology"]` — if exists and < 5 min old, hydrate the store immediately (instant render).
...
Every N seconds (configurable, default 10s):
  ...
  For each measurement:
    Append {timestamp, value} to ring buffer
        ...
        └──► localStorage "twinlight-ui:series:{metric}:{uuid}"
...
- "Clear Cache" button — purges all `twinlight-ui:*` keys from localStorage.
...
All keys are prefixed with `twinlight-ui:` to avoid collisions.

| Key | Value | Purpose |
|-----|-------|---------|
| `twinlight-ui:settings` | `{ url, pollInterval, ... }` | Connection settings |
| `twinlight-ui:topology` | `{ data, fetchedAt, etag? }` | Cached topology JSON |
| `twinlight-ui:series:{metric}:{serviceUuid}` | `[{t, v}, ...]` | Ring buffer for one metric of one service |
| `twinlight-ui:series:index` | `[{metric, serviceUuid, count}]` | Index of all active series (for enumeration) |
...
1. Initialize project: `npm create vite@latest twinlight-ui -- --template react-ts`
